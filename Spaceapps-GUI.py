...
def Schedule():
    ...
    blue_label=Label(Schedule,text="-SLEEP").place(x=923,y=900)
    
    im22=Image.open('yellow.png')
    im23=im22.resize((20,20))
    im24=ImageTk.PhotoImage(im23)
    Key_yellow=Label(Schedule,image=im24).place(x=800,y=500)
    yellow_label=Label(Schedule,text='-EXERCISE').place(x=823,y=900)
    Key_blue=Label(Schedule,image=im21).place(x=900,y=500)
    while(count<6):
    ...
    if(Gender_input.get()=='Male'):
        BEE= 66 + (13.7 * (int(Integer.get())) + (5 * (int(Height_input()))) - (6.8 * (int(Age_input.get()))))
        Nutrition=Label(Schedule,text="Daily Nutrition Consumption={} kcal/day".format(BEE)).place(x=600,y=700)
    elif(Gender_input.get()=='Female'):
        BEE= 655 + (9.6 * (int(Integer.get())) + (1.7 * (int(Height_input()))) - (4.7 * (int(Age_input.get()))))
        Nutrition=Label(Schedule,text="Daily Nutrition Consumption={} kcal/day".format(BEE)).place(x=600,y=700)            
    Schedule.mainloop()
